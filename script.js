...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function nearestGrade(wpm){
    const cpm=wpm*5;
    let closest=gradeTable[gradeTable.length-1];
    let minDiff=Math.abs(cpm-closest.cpm);
    for(const g of gradeTable){
      const diff=Math.abs(cpm-g.cpm);
  ...
  /* ==== テストロジック ==== */
  let countdownInterval,timerInterval,startTime=0,limitSeconds=60,currentPrompt='';

  function startTest(){
    resultEl.textContent='';
    inputEl.value='';
    inputEl.disabled=true;
    timerEl.textContent='';countdownEl.textContent='';
    ...
    countdownInterval=setInterval(()=>{
      ...
      else{clearInterval(countdownInterval);countdownEl.textContent='スタート!';beginTyping();}
    },1000);
  }
  ...
  function beginTyping(){
    ...
    timerInterval=setInterval(()=>{
      secLeft--;
      if(secLeft>0){timerEl.textContent=`残り ${secLeft} 秒`;}
  ...
  function finishTest(){
    ...
    const grade=nearestGrade(Number(wpm));

    resultEl.textContent=`正確さ: ${accuracy}% | WPM: ${wpm} | 近い級: ${grade}`;
    saveRecord(wpm,accuracy,grade);
    renderHistory();
  ...
  function saveRecord(wpm,acc,grade){
    const rec={date:new Date().toLocaleString('ja-JP'), wpm:Number(wpm), acc:Number(acc), grade};
    const arr=getRecords(); arr.push(rec);
  ...
  function renderHistory(){
    ...
    arr.forEach(r=>{
      ...
      tr.innerHTML=`<td>${r.date}</td><td>${r.wpm}</td><td>${r.acc}</td><td>${r.grade}</td>`;
      tableBody.appendChild(tr);
    });
  }
  clearBtn.onclick=()=>{if(confirm('履歴を削除しますか？')){localStorage.removeItem('records');renderHistory();}};

  /* === イベント === */
  startBtn.onclick=()=>{startBtn.disabled=true;startTest();};
  renderHistory();
});
